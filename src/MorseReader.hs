...
import Data.WAVE
    ( getWAVEFile, WAVE(WAVE), WAVEHeader(WAVEHeader), WAVESample )
import Data.List ( foldl', group, partition, sort, sortOn )
import qualified Data.Ord

data Morse = Dit | Dah | Space | LetterSpace | WordSpace
    deriving (Eq)
...
readMorseWAVEfile filepath = do
    (WAVE (WAVEHeader channels framerate bits frames) samples) <- getWAVEFile filepath
    ...
    let onesAndZeroes = map (\a -> if a`div` 5 > 2 then 1 else 0) quantizedSlices
    ...
    let [wordSpaceMinLength, letterSpaceMinLength] = take 2 $ map snd . sortOn (Data.Ord.Down . fst) $ distancesBetweenNeighbouringValues spaceLengthsSorted

    let morses = sequencesToMorses dahMinLength wordSpaceMinLength letterSpaceMinLength lengths
...
sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ((1,l):ls)
    | l < sigLength = Dit : sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ls
    | otherwise = Dah : sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ls
sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ((0,l):ls)
    | l < letterSpaceMinLength = Space : sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ls
    | l < wordSpaceMinLength = LetterSpace : sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ls
    | otherwise = WordSpace : sequencesToMorses sigLength wordSpaceMinLength letterSpaceMinLength ls
sequencesToMorses _ _ _ _ = []

distancesBetweenNeighbouringValues :: (Eq b, Num b) => [b] -> [(b, b)]
distancesBetweenNeighbouringValues (x:x':xs) = foldl (\a@((n,i):acc) nxt -> if nxt == i then a else (nxt-i, nxt):a) [(x-x',x')] xs
...
avg samples = fromIntegral (absAverage' samples) / fromIntegral (length samples)
  where absAverage' samples = foldl' (\acc nxt -> (acc`div`10000) + abs nxt) 0 samples

-- Should be using this instead of avg, but ran out of time
rms' :: (Integral a) => [a] -> Double
rms' [] = 123456789.0
rms' xs = sqrt $ (fromIntegral $ (sum . map ((^2) . (`div`10000))) xs) / fromIntegral (length xs)
...
parseMorse ms = let (letter, ms') = parseMorseChar ms
                           in letter : parseMorse ms'

parseMorseChar :: [Morse] -> (Char, [Morse])
parseMorseChar [] = ('.', [])
parseMorseChar ms = let (didaas, ms') = span (\m -> m==Dit || m==Dah) ms
               in (fromMaybe '*' (lookup didaas morseAlphabet), ms')

translateMorse :: [Morse] -> [Char]
translateMorse = parseMorse . convertSpaces . stripSpacesBetween . stripStartAndEndSpaces
